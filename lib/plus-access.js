...
function issuePlusToken({
  ...
  const suppliedExpiryMs = new Date(expiresAt || "").getTime();
  if (expiresAt && !Number.isFinite(suppliedExpiryMs)) {
    const error = new Error("Scadenza pass non valida.");
    error.publicMessage = "Non riesco a verificare la durata del pass.";
    error.statusCode = 502;
    throw error;
  }
  const expiresAtMs = Number.isFinite(suppliedExpiryMs)
    ? suppliedExpiryMs
    : paidAtMs + ACCESS_DAYS * 24 * 60 * 60 * 1000;
  const payload = {
    v: TOKEN_VERSION,
    ...
    source: source === "manual_admin" ? "manual_admin" : "stripe",
...
async function requirePlusAccess(req, user) {
  const access = verifyPlusToken(plusTokenFromRequest(req), user);
  if (access.active) {
    const { loadPlusEntitlement } = require("./plus-entitlements");
