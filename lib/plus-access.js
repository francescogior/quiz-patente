...
function plusSecret() {
  ...
  if (!value) {
    const error = new Error("SUPABASE_SERVICE_ROLE_KEY mancante.");
...
function sign(payloadPart) {
  return crypto.createHmac("sha256", plusSecret()).update(payloadPart).digest("base64url");
}
...
function verifyPlusToken(token, user) {
  const [payloadPart, signature, extra] = String(token || "").trim().split(".");
...
function requirePlusAccess(req, user) {
  ...
  if (!access.active) {
    ...
    error.publicMessage =
      "Questa nuova spiegazione richiede Quiz Patente Plus. I contenuti già disponibili restano gratuiti.";
