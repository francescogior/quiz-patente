...
} = require("./plus-admin-grants");

const BUCKET = "quizpatente-plus-entitlements";

async function loadPlusEntitlement(user) {
  const [paid, manual] = await Promise.all([
    loadPaidPlusEntitlement(user),
    loadActiveManualPlusGrant(user),
  ]);
  return selectEffectiveEntitlement(paid, manual);
}

async function loadPaidPlusEntitlement(user) {
  try {
...
async function loadPlusEntitlementsForUsers(users = []) {
  const entries = users
    .filter((user) => user?.id && user?.email)
    .map((user) => ({ user, path: entitlementPath(user) }));
  if (entries.length === 0) return {};

  try {
    const [rows, manualByUser] = await Promise.all([
      query(
        `select object_key, payload
         from app_kv_objects
        where namespace = $1
          and object_key = any($2::text[])`,
        [BUCKET, entries.map((entry) => entry.path)],
      ),
      loadManualPlusGrantsForUsers(entries.map((entry) => entry.user)),
    ]);
    const payloadByPath = new Map(
      rows.map((row) => [row.object_key, normalizeEntitlement(row.payload)]),
    );
    return Object.fromEntries(
      entries.map(({ user, path }) => [
        String(user.id),
        selectEffectiveEntitlement(
          payloadByPath.get(path) || null,
          manualByUser[String(user.id)] || null,
        ),
      ]),
    );
  } catch (error) {
...
function selectEffectiveEntitlement(...entitlements) {
  const valid = entitlements.filter(Boolean);
  if (valid.length === 0) return null;
  const active = valid.filter((entitlement) => entitlement.active);
  const candidates = active.length > 0 ? active : valid;
  return candidates.reduce((best, entitlement) => {
    if (!best) return entitlement;
    return new Date(entitlement.expiresAt).getTime() >
      new Date(best.expiresAt).getTime()
      ? entitlement
      : best;
  }, null);
}
...
module.exports = {
  ...
  loadPaidPlusEntitlement,
  loadPlusEntitlement,
  loadPlusEntitlementsForUsers,
