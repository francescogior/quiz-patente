...
async function createLoginCode(email) {
  ...
  const activeCodesUrl = new URL(CODES_TABLE, "https://example.test");
  activeCodesUrl.searchParams.set("email", `eq.${email}`);
  activeCodesUrl.searchParams.set("consumed_at", "is.null");
  await supabaseFetch(`${activeCodesUrl.pathname.slice(1)}${activeCodesUrl.search}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
...
async function verifyLoginCode(email, code) {
  ...
  const consumeUrl = new URL(CODES_TABLE, "https://example.test");
  consumeUrl.searchParams.set("id", `eq.${match.id}`);
  consumeUrl.searchParams.set("consumed_at", "is.null");
  consumeUrl.searchParams.set("expires_at", `gt.${new Date().toISOString()}`);
  consumeUrl.searchParams.set("select", "id,user_id");
  const consumed = await supabaseFetch(
    `${consumeUrl.pathname.slice(1)}${consumeUrl.search}`,
    {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
