...
const { loadManualPlusGrantAudit } = require("./plus-admin-grants");
const { loadPlusEntitlementsForUsers } = require("./plus-entitlements");
...
async function getAdminDashboard() {
  ...
  const [plusByUser, manualPlusGrants] = await Promise.all([
    loadPlusEntitlementsForUsers(users),
  ...
  const adminUsers = users.map((user) => {
    ...
    return {
      ...
      plus: adminPlusState(plusByUser[String(user.id)]),
    };
  });
  ...
  const activity = [
    ...
    ...manualPlusGrants
      ...
      .map((grant) => ({
        type: "plus_grant",
        label: "Plus manuale",
        at: grant.createdAt || grant.validFrom,
        userId: grant.targetUserId,
        email: usersById.get(grant.targetUserId)?.email || "utente sconosciuto",
        detail: `Pass di ${grant.durationDays} giorni`,
      })),
  ]
    .filter((item) => item.at)
  ...
  return {
    summary: {
      ...
      tests: totalTests,
      passedTests,
      plusUsers: adminUsers.filter((user) => user.plus.active).length,
...
function adminPlusState(entitlement) {
  if (!entitlement) {
    return {
      active: false,
      expiresAt: null,
  ...
  return {
    ...
    revocationReason: entitlement.revocationReason || null,
  };
}
