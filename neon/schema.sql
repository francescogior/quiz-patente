...
create table if not exists app_kv_objects (
...
create index if not exists app_login_codes_lookup_idx
  on app_login_codes (email, code_hash, expires_at desc)
  where consumed_at is null;

create index if not exists app_sessions_lookup_idx
  on app_sessions (token_hash, expires_at desc);

create index if not exists user_exam_results_user_finished_idx
  on user_exam_results (user_id, finished_at desc);
